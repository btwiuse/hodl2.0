...
function Dashboard(){
  ...
  
  {
    ...
    web3.eth.getBalance("0xc892eda63D52cF680EE06882Dc4960BF24338740")
        .then(balance => {
    ...
    contractPool.methods.stopcampaign().call().then(stopTime => {
...
//    console.log("1",a); 
    ...
    return ( 
    <> 
<a href="" class="fixedbutton" data-toggle="modal" data-target="#InvestMent">Reward Calculator</a>
    <section class="smallsection">
<div class="modal fade  " id="InvestMent" tabindex="-1" aria-labelledby="InvestMentLabel" aria-hidden="true">
   <div class="modal-dialog modal-lg ">
      <div class="modal-content modal_hodlInsvetmentcal">
         <div class="modal-header">
            <h5 class="modal-title" id="InvestMentLabel">HODL Investment & Rewards Calculator</h5>
               ...
               <div class="col-md-12">
                  <form>
                     <div class="form-group row">
                        <label for="inputPassword" class="col-sm-6 col-form-label">Enter BNB Amount to Invest</label>
                        {/* <input type="text" class="form-control" id="text" placeholder="Enter BNB Amount" onChange={e => setValues(e.target.value)}  /> */}
                        <div class="col-sm-6">
                           <input type="text" class="form-control" id="inputtext" onChange={e => setValues(e.target.value)} />
                        </div>
                     </div>
                  </form>                     
               </div>
                  ...
                  <table class="table table-bordered table-striped text-center">
                     ...
                     <tbody>
                        <tr> 
                           ...
                           <td>{new Intl.NumberFormat().format(Number(LMBalanceuser).toFixed(0))}</td>
                           <td>{new Intl.NumberFormat().format(hodl)}</td>
                           <td>{new Intl.NumberFormat().format(Number(Number(hodl).toFixed(0))+Number(Number(LMBalanceuser).toFixed(0)))}</td>
                        </tr>
                        <tr>
                           <th scope="row">Reward Share %</th>
                           <td>{( (Number(bnbreward) / Number(TotalbnbinrewardPool) ) * 100).toFixed(3)} %</td>
                           <td>{Number(((dailyreward/TotalbnbinrewardPool)*100)).toFixed(3)} %</td>



<td>{(( (Number(bnbreward) / Number(TotalbnbinrewardPool) ) * 100)+ Number((dailyreward/TotalbnbinrewardPool*100))).toFixed(3)} %</td>
                        ...
                        <tr>
                           ...
                           <td>{(Number(bnbreward)+ Number(dailyreward)).toFixed(3)}</td>
                        </tr>
                        <tr>
                           <th scope="row">Yearly Rewards in BNB*</th>
                           <td>{(Number(bnbreward) * 364).toFixed(2)}</td>
                           <td>{Number(yearly).toFixed(2)}</td>
                           <td>{Number(Number(Number(bnbreward) * 364) + Number(yearly)).toFixed(2)}</td>
                        </tr>
                     </tbody>
                  </table>
               </div>
               <div class="button_cacla"> 
                  <a href="https://exchange.pancakeswap.finance/#/swap?outputCurrency=0x0E3EAF83Ea93Abe756690C62c72284943b96a6Bc" target ="_blank" class="btn-get-started scrollto">Buy HODL</a>
               </div>
    ...
    <section id="services" className="services section-bg section_feature dashboard_page mb-0 pb-0 pt-2">
                <div className="container" data-aos="fade-up">
                    <div className="row mt-3">
                      ...
                      <h5>My collectable BNB: {Number(bnbreward).toFixed(5)} BNB  </h5>   
...
<h5 style={{ fontSize: '1.00rem'}} > Minimum Balance: {new Intl.NumberFormat().format(startbalance)}</h5>
<h5 style={{ fontSize: '1.00rem'}} > Maximum Balance: {new Intl.NumberFormat().format(endbalance)}</h5>
...
{ ( (collectiontime < Math.round(Date.now() / 1000)) || collectiontime == Number(0))?
<a onClick = {claimextraBNB} class="reward_button " data-toggle="modal" data-target="#RewardsButton">Claim BNB</a>
:
<a onClick = {()=> {swalreact(
        <div>
        <h5>"Please wait till your collectible date is reached"</h5>
        <p>Your Collectible Date is {  new Date(collectiontime* 1000).toLocaleString("en-US",{weekday: "long"})}, {new Date(collectiontime* 1000).toLocaleString("en-US",{month: "long"})} {new Date(collectiontime* 1000).toLocaleString("en-US",{day: "numeric"})}, {new Date(collectiontime* 1000).toLocaleString("en-US",{year: "numeric"})}  {new Date(collectiontime * 1000).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })  }</p>
             
        </div>
        )}} 
class="reward_button " data-toggle="modal" data-target="#RewardsButton">Claim BNB</a>
}
